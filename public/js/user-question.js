...
var UserQuestion = function () {
    ...
    $(document).on('click', '#answer_submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // $("#textresult").html(response.textresult);
                //swal("success", response.textresult, "success");

                $('#create-reports').click();
            },
    ...
    $(document).on('change', '#files',function(e) {
        ...
        var default_number_files = $(this).siblings('#num_files').val() ?? 1;
        ...
        data.append('totalsize',file_size);
//        data.append('')
        ...
        if(all_number <= default_number_files){
            ...
            $.ajax({
                ...
                processData: false,
                success: function(response) {
                    template_alert('File salvati con successo',response.result);
    ...
    var display_image = function (image_name, qt) {
        ...
        if(image_name['type'] == 'image'){
            var html_append = `<div class="preview-img clearfix mt-3" data-question="`+qt+`" data-file="`+image_name['name']+`"><img src='`+default_path+`/`+image_name['name']+`' alt=`+image_name['name']+` class="mb-5 float-left"><a class="preview-del-button gradient-bg float-right" id="del_file"  style="text-align:center !important; background:${$(`#q_${qt} #btn_color`).val()};${$(`#q_${qt} #font_style`).val()}" href="javascript:void(0)">Delete</a></div>`;
        }else {
            var html_append = `<div class="preview-files clearfix mt-3" data-question="`+qt+`" data-file="`+image_name['name']+`"><a href='`+default_path+`/`+image_name['name']+`' class="mb-5" download style="${$(`#q_${qt} #font_style`).val()}">`+image_name['name']+`</a><a class="preview-del-button gradient-bg float-right" id="del_file" style="text-align:center !important;background:${$(`#q_${qt} #btn_color`).val()};${$(`#q_${qt} #font_style`).val()}" href="javascript:void(0)">Delete</a></div>`;
        }
    ...
    $(document).on('click', '#create-reports', function (e) {
        ...
        if (flag != false) {
            ...
            $.ajax({
                data: {test_id: test_id, test: arr, reported: 1,course_id:course_id},
                url: "../get_report",
                type: "POST",
                dataType: "json",
                success: function (response) {
                    ...
                    chartids = [];
                    types = [];
                    console.log("----test id ---", test_id, arr, course_id);
                    ...
                    for (var i = 0; i < con_str.length; i++) {
                        if (con_str[i].includes("[text id=") == true) {
                            ...
                            var text_val = parseInt(text_index.split(["]"])[0]);
                            
                            //console.log(textgroup_data);
                            Object.values(textgroup_data).forEach((textgroup) => {
                                //console.log(textgroup, text_val);

                                if (text_val == textgroup['id']) {
                                    textgroup_ids.push(text_val);
                                    content_str = $("#report").html();

                                    //let tg_html = '<div id="textgroup'+text_index+'" class="col-12">'+tg_content+'</div>';
                                    content_str = content_str.replace("[text id=" + text_val + "]", textgroup['content']);
                            ...
                            if(tg_idx == 0) {
                                content_str = content_str.replace("[text id=" + text_val + "]", '');
                                $("#report").html($.parseHTML(content_str));
                            }
                        }
                        //chart part
                                    ...
                                    if (chart_val == response.chart_data[j]['id']) {
                                       ...
                                       //contents.push(content);
                                        ...
                                        if (chart_type == 0) {
                                            ...
                                            content_str = content_str.replace('[chart id=' + chart_val + ']', ' <div class="col-12 chartcontainer"><canvas id="myChart"></canvas></div>');
                                            $("#report").html($.parseHTML(content_str));
                    ...
                    setTimeout(function () {
                        ...
                        $("#update_report_div").css('display', '');
                        $("#update_report_div").removeAttr('style');
                    },2000);
                },
    ...
    var drawChart = function (contents, chartids, types) {
        console.log("show my report content",contents)
        $("#report").hide();
        let idx = 1;
    ...
    function CreatePDFfromHTML(classDiv,test_id) {

  var HTML_Width = $("." + classDiv).width();
  var HTML_Height = $("." + classDiv).height();
  ...
  html2canvas($("." + classDiv)[0]).then(function (canvas) {
    ...
    var randomString = generateRandomString(5); // Generate a random string
    var fileName = classDiv + "_" + randomString + ".pdf"; // Append the random string to the filename
    
    var formData = new FormData();
    formData.append('pdf', pdf.output('blob'), fileName);
    formData.append('test_id', test_id);
    formData.append('classDiv', classDiv);
    ...
    $.ajax({
      ...
      success: function(response) {
        //   setTimeout(function () {
                $("#loader").fadeOut();
            // }, 10000);
        template_alert("Success", response.success);
      },
      error: function(error) {
          setTimeout(function () {
            $("#loader").fadeOut();
...
function generateRandomString(length) {
  ...
  return result;
    ...
    // function CreatePDFfromHTML(classDiv) {
    //     var HTML_Width = $("."+classDiv).width();
    //     var HTML_Height = $("."+classDiv).height();
    //     var top_left_margin = 15;
    //     var PDF_Width = HTML_Width + (top_left_margin * 2);
    ...
    $(document).on('click', '#update-report', function (e) {
        ...
        var firstFontSize = $("#firstFontSize").val();
        var firstStyle = $("#firstStyle").val();
        var firstFamily = $("#firstFamily").val();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $(".alert-content").css('font-size',''+firstFontSize+'');
                $('.alert-content').css('font-style', ''+firstStyle+'');
                $('.alert-content').css('font-family', ''+firstFamily+'');

                    
                $("#report_div").css('display', '');
                ...
                CreatePDFfromHTML("question-ans",test_id);
                CreatePDFfromHTML("report-card",test_id);
            },
            error: function (error) {
        ...
        if(type == 0){
            ...
            for(var k = 0; k < rates.length; k++){
                if($(rates[k]).data('value') == ans[0].answer){
                    $(rates[k]).css('background',`var(--qcolor2, ${color})`);
                }
            }
            $(question).children().find('#displayed_answer').val(1);
        } else if(type == 6){
            ...
            $(question).children().find('input[type=text]').val(ans[0].answer);
            $(question).children().find('#displayed_answer').val(1);
        }
    }
    ...
    var collecting_answers = function () {
        ...
        $.each(ranges, function (i, v) {
            if ($('input[data-id="' + v + '"]').val() != "") {
                ...
                obj.key = v;
                obj.qid = v;
                obj.value = parseInt($('input[data-id="' + v + '"]').val());
                arr.push(obj);
                // arr[v] =$('input[name^=radiogroup]:checked').val().trim();   
            }
        });
    ...
    var storeData = function (test_id) {
        ...
        $.ajax({
            ...
            success: function (response) {
                //updated by Polaris
                // if (response.textgroup_data!='')
                // {
                //     $('#modelHelp').html(response.textgroup_data);
                //     $('#helpModel').modal('show');
                // }
                if (response.textgroup_data!=null && response.textgroup_data!='') {
                    $('#report').html(response.textgroup_data);
                    $(".card-body").css({"overflow":"auto"});
                }
            }
        })
    ...
    var show_progress = function (){
        ...
        // console.log("show my parcet :: ",percent);
        $('#percent').val(percent);
        $('.b_progress_bar').css({'width': percent.toFixed(2)+'%'});
        $('.b_progress').html(Math.round(percent)+'%').css({'left': 'calc(' + percent.toFixed(2)+'% - 22px)'});
    }
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(chartData != null){
                ...
                $(`${content} table`).css("width",chartData.table.width+"px");
                $(`${content} table`).css("height",chartData.table.height+"px");
...
function line_stacked_draw(content, cid,idx){
    ...
    $("#report .col-12.chartcontainer").append(canvasElem);
        ...
        success: function(response){
            ...
            if(chartData != null){
                ...
                for(var i=0;i<Object.keys(chartData.chart).length;i++)
                    myChart.data.datasets[i].backgroundColor=convertToTransparentColor(chartData.chart[i]);
