...
var UserQuestion = function () {
    ...
    $(document).ready(function (e) {
        $('#loader').show();
              ...
              // Call sendPageAsHtml after your previous AJAX is done
...
//        data.append('')
    ...
    var display_image = function (image_name, qt) {
        ...
        if(image_name['type'] == 'image'){
            var html_append = `<div class="preview-img clearfix mt-3" data-question="`+qt+`" data-file="`+image_name['name']+`"><img src='`+default_path+`/`+image_name['name']+`' alt=`+image_name['name']+` class="mb-5 float-left"><a class="preview-del-button gradient-bg float-right" id="del_file"  style="background:${$(`#q_${qt} #btn_color`).val()};${$(`#q_${qt} #font_style`).val()}" href="javascript:void(0)">Delete</a></div>`;
        }else {
            var html_append = `<div class="preview-files clearfix mt-3" data-question="`+qt+`" data-file="`+image_name['name']+`"><a href='`+default_path+`/`+image_name['name']+`' class="mb-5" download style="${$(`#q_${qt} #font_style`).val()}">`+image_name['name']+`</a><a class="preview-del-button gradient-bg float-right" id="del_file" style="background:${$(`#q_${qt} #btn_color`).val()};${$(`#q_${qt} #font_style`).val()}" href="javascript:void(0)">Delete</a></div>`;
        }
...
//       $("#loader").fadeOut();
//     }, 15000);
//         // Handle the success response from the server, such as updating the table with the file name
//         console.log(response);
//       },
//       error: function(error) {
//           setTimeout(function () {
...
// async function sendPageAsHtml() {
//   try {
//     // Capture the image of the HTML content using html2canvas
//     const canvas = await html2canvas(document.documentElement);
//     const imageData = canvas.toDataURL();

//     // Send the image data to the server using AJAX
//     const response = await $.ajax({
//       url: '/pdf/generate-pdf', // Replace with your server-side API endpoint
//       method: 'POST',
//       data: {
...
function sendPageAsHtml() {
  $('#loader').hide();

  // Capture the image of the HTML content using html2canvas
  html2canvas(document.documentElement).then(function(canvas) {
    // Convert the canvas to a blob with the specified MIME type (image/jpeg)
    canvas.toBlob(function(blob) {
      // Use FileReader to read the blob data
      var reader = new FileReader();

      // Define the function to be executed when the blob is read
      reader.onloadend = function() {
        // Get the base64-encoded compressed image data from the FileReader result
        var compressedImageData = reader.result;

        $('#loader').show();

        // Set the compressed image data to the hidden input field in the form
        $('#image_data').val(compressedImageData);

        // Submit the form
        $('#pdfform').submit();
      };

      // Use FileReader to read the blob as a Data URL (base64-encoded image)
      reader.readAsDataURL(blob);
    }, 'image/jpeg', 0.8); // 'image/jpeg' is the MIME type and 0.8 is the JPEG quality
