...
<?php $__env->startSection('title', __('Test Report').' | '.app_name()); ?>
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('question_create')): ?>
                <div class="float-right">
                   <a id="order_change" 
                    ...
                    <a href="<?php echo e(route('admin.testreports.create', [
...
<script>
        ...
        jQuery(document).ready(function (e) {
           // QuestionEdit.init();
            ...
            var route = '<?php echo e(route('admin.testreports.get_data')); ?>';

            <?php if(request('show_deleted') == 1): ?>
                route = '<?php echo e(route('admin.testreports.get_data',['show_deleted' => 1])); ?>';
            <?php endif; ?>

            <?php if(request('test_id') != ""): ?>
                route = '<?php echo e(route('admin.testreports.get_data',['test_id' => request('test_id')])); ?>';
            <?php else: ?>
                <?php if(request('course_id') != ""): ?>
                    route = '<?php echo e(route('admin.testreports.get_data',['course_id' => request('course_id')])); ?>';
                <?php endif; ?>
            <?php endif; ?>
            ...
            $(document).on('change', '#test_id', function (e) {
                ...
                window.location.href = "<?php echo e(route('admin.testreports.index')); ?>" + "?course_id=" + course_id + "&test_id=" + test_id;
            });
            $(document).on('change', '#course_id', function (e) {
                var course_id = $(this).val();
                window.location.href = "<?php echo e(route('admin.testreports.index')); ?>" + "?course_id=" + course_id;
            });

            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('question_delete')): ?>
            <?php if(request('show_deleted') != 1): ?>
            $('.actions').html('<a href="' + '<?php echo e(route('admin.testreports.mass_destroy')); ?>' + '" class="btn btn-xs btn-danger js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">Delete selected</a>');
            <?php endif; ?>
            <?php endif; ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('backend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\FreelancerTask\Laravel\diagnosi12_05_01\resources\views/backend/testreports/index.blade.php ENDPATH**/ ?>